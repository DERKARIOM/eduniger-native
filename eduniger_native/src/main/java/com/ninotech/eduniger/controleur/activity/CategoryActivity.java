...
public class CategoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSwipeRefreshLayout = findViewById(R.id.swipe_refresh_category);  // ← nouveau
        mList             = new ArrayList<>();
        ...
        // BroadcastReceiver existant — inchangé
        ...
        if (mNameStruct == null) {
            ...
            new StructCategoryInSyn().execute(
                    Server.getUrlApi(getApplicationContext()) + "StructCategoryIn.php",
                    mSession.getIdNumber(),
                    mCategorie,
                    mNameStruct);
        }
            ...
            if (jsonData != null) {
                ...
                if (!jsonData.equals("RAS")) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        try {
                            mList.add(new OnlineBook(
                                    jsonArray.getJSONObject(i).getString("idBook"),
                                    jsonArray.getJSONObject(i).getString("blanket"),
                                    jsonArray.getJSONObject(i).getString("bookTitle"),
                                    jsonArray.getJSONObject(i).getString("nameStruct") + " : " +
                                            jsonArray.getJSONObject(i).getString("categoryTitle"),
                    ...
                    voidContainer(R.drawable.img_telecharge_local,
                            "Aucun livre de categorie " + mCategorie);
                }
            } else {
                ArrayList<Connection> list = new ArrayList<>();
